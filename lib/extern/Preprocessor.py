...
class Preprocessor:
  ...
  def handleLine(self, aLine, liter):
    """
    Handle a single line of input (internal).
    """

    if liter and self.disableLevel == 0:
      self.write(aLine)
      return
  ...
  # File ops
  def do_include(self, args, filters=True, liter=False):
    """
